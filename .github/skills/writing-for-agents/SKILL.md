...
description: Writing documents for agents. Use when creating or editing a skill, prompt, or custom agent, or modifying AGENTS.md or copilot-instructions.md.
---

Reference for writing any document an agent consumes — a skill, a prompt, a custom agent, an `AGENTS.md` / `.github/copilot-instructions.md`, a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable — the agent taking the same _process_ every run, not producing the same output.

When the document you're writing is a skill, prompt, or agent, read [`SKILL-MECHANICS.md`](SKILL-MECHANICS.md) for frontmatter, invocation choice, and router skills.
