...
If this repo treats external pull requests as a request surface (a `PRs as a request surface: yes` line in `.github/unisquirl.md`), triage covers them too: **a PR is an issue with attached code** — same roles, same states, same machine, with a few deltas marked "for a PR" below. Resolve a bare `#42` to an issue or PR per the tracker reference.
...
These are canonical role names — the actual label strings may differ. Read [triage-labels.md](triage-labels.md) for the defaults, and `.github/unisquirl.md` for this repo's overrides if it has any.
